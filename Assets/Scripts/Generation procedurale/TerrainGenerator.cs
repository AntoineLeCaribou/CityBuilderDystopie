...
public class TerrainGenerator : MonoBehaviour
{
    ...
    private static Grid AjouterRiviere(Grid grille, int largeur, int hauteur, GridBuildingSystem systemeDeBuild)
    {
        ...
        systemeDeBuild.Placer(Encyclopedie.Riviere[0], xdebut, ydebut, Encyclopedie.Riviere[0].GetComponent<ShopItem>().id, Encyclopedie.Riviere[0].GetComponent<ShopItem>().nomDeLobjet, false);
        ...
        while (currentx > 0 && currenty > 0 && currentx < largeur && currenty < hauteur)
        {
            ...
            {
                systemeDeBuild.Placer(Encyclopedie.Riviere[0], currentx, currenty, 2, Encyclopedie.Riviere[0].GetComponent<ShopItem>().nomDeLobjet, false);
            }
            ...
            if (nbAleatoire >= 3)
            {
                ...
                if (grille.GetValue(currentx, currenty) > 0 || GetNbVoisins(currentx, currenty, grille, "Eau") >= 2)
                {
                    currentx -= mouvementx;
                    currenty -= mouvementy;
                }
    ...
    public static Grid RefreshSol(Grid grille, int x, int y, int largeur, int hauteur, GridBuildingSystem systemeDeBuild)
    {
        ...
        if (grille.GetNom(x, y) == "Eau")
        {
            ...
            if (grille.GetNom(x + 1, y) != "Eau" && grille.GetValue(x + 1, y) != -1)
            {
                rotationY = -90;
                rot = Quaternion.Euler(new Vector3(0, rotationY, 0));
                systemeDeBuild.PlacerSol(prefab, x, y, rot, true);
            ...
            if (grille.GetNom(x, y + 1) != "Eau" && grille.GetValue(x, y + 1) != -1)
            {
                rotationY = -180;
                rot = Quaternion.Euler(new Vector3(0, rotationY, 0));
                systemeDeBuild.PlacerSol(prefab, x, y, rot, true);
        ...
        if (grille.GetNom(x, y) == "Route")
        {
            ...
            int nbVoisins = GetNbVoisins(x, y, grille, "Route");
    ...
    private static int GetNbVoisins(int x, int y, Grid grille, string nom)
    {
        int i = 0;

        if (grille.GetNom(x + 1, y) == nom)
            i++;

        if (grille.GetNom(x - 1, y) == nom)
            i++;

        if (grille.GetNom(x, y + 1) == nom)
            i++;
    ...
    private static Grid AjouterVegetation(Grid grille, int largeur, int hauteur, float[,] matriceDeBruit, GridBuildingSystem systemeDeBuild)
    {
        for (int x = 0; x < largeur; x++)
        {
            for (int y = 0; y < hauteur; y++)
            {
                //si il y a déjà un truc
                if (grille.GetValue(x, y) > 0)
                    continue;

                //on ne met rien
                if (matriceDeBruit[x, y] >= 0.3f && matriceDeBruit[x, y] <= 0.55f)
                    continue;
                ...
                if (matriceDeBruit[x, y] > 0.55f)
                    prefab = Encyclopedie.GetObjetAleatoire(Encyclopedie.Arbres);

                //Buissons
                else if (matriceDeBruit[x, y] < 0.15f)
                    prefab = Encyclopedie.GetObjetAleatoire(Encyclopedie.Buissons);

                //Rochers
                else
                    prefab = Encyclopedie.GetObjetAleatoire(Encyclopedie.Rochers);

                ShopItem info = prefab.GetComponent<ShopItem>();

                systemeDeBuild.Placer(prefab, x, y, info.id, info.nomDeLobjet, true);
            }
        }
